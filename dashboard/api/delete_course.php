...
if (!$id) { flashMessage('error', 'מזהה קורס חסר.'); redirect('/dashboard/clients/list.php'); }
...
$course->execute([$id]);
$course = $course->fetch();
if (!$course) { flashMessage('error', 'קורס לא נמצא.'); redirect('/dashboard/clients/list.php'); }
...
redirect("/dashboard/clients/view.php?id={$clientId}");
