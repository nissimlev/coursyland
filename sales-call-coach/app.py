...
REPORT_SECTION_TITLES = [
    "כשלים מרכזיים",
    "ציטוטים מהשיחה ומה היה אפשר לנצל",
    "מה איש המכירות עשה לא טוב",
    "מה היה אפשר לעשות טוב יותר ואיך",
    "שאלות חובה שלא נשאלו",
    "כאבים שהלקוח העלה והאם העמיקו אותם",
...
def build_analysis_prompt(
    ...
    return f"""
נתח שיחת מכירה בעברית כמו מנהל מכירות חד ומנוסה, לא כמו דוח כללי.
המטרה: לתת לאיש/אשת המכירות משוב שאפשר ללמוד ממנו מיד: מה לא עבד, למה, מה היה אפשר לנצל, ואיך לומר זאת טוב יותר בפעם הבאה.
...
הנחיות:
- אל תכתוב מחמאות כלליות כמו "השיחה הייתה טובה", "היה אמון", "הוצג פתרון". כל טענה חייבת להיות מחוברת לציטוט או רגע ספציפי מהשיחה.
- פתח את הדוח ישר ב"כשלים מרכזיים". אל תתחיל בתקציר.
- ב"כשלים מרכזיים" כתוב 3-5 כשלים חדים. לכל כשל:
  - תן כותרת נוקבת, למשל "התאהבות בסיפור במקום חקירת העסק".
  - הבא ציטוט קצר מהשיחה שמוכיח את הכשל.
  - הסבר למה זה פגע במכירה.
  - כתוב מה היה צריך לעשות במקום.
- ב"ציטוטים מהשיחה ומה היה אפשר לנצל" הבא לפחות 5 ציטוטים משמעותיים מהלקוח או מאיש המכירות.
  לכל ציטוט כתוב: "מה היה אפשר לנצל כאן" ו"תשובה/שאלה טובה יותר".
- ב"מה איש המכירות עשה לא טוב" כתוב באופן ישיר, בלי ריכוך מיותר. התמקד בהתנהגות מכירתית שניתנת לשיפור.
- ב"מה היה אפשר לעשות טוב יותר ואיך" תן ניסוחים חלופיים ממשיים שאפשר לומר בשיחה.
- ב"שאלות חובה שלא נשאלו" כתוב שאלות שאיש/אשת המכירות היו חייבים לשאול, לפי ההקשר. למשל על תקציב, קהל, דחיפות, השלכות אי-פעולה, ניסיון קודם, מקבל החלטה, יכולת תשלום.
- ב"כאבים שהלקוח העלה והאם העמיקו אותם" כתוב:
  - הכאב כפי שעלה מהשיחה.
  - ציטוט שמוכיח אותו.
  - האם איש/אשת המכירות העמיק/ה את הכאב.
  - איזו שאלת העמקה היתה צריכה להישאל.
...
- ב"הובלת השיחה" השורה הראשונה חייבת להיות בדיוק בפורמט: ציון הובלה: 70/100
  לאחר מכן נתח האם איש/אשת המכירות הוביל/ה את השיחה או נסחף/ה אחרי הלקוח. כתוב באחוזים משוערים כמה זמן היה חקירה וכמה זמן היה הסבר.
- ב"סיכוי משוער לסגירת העסקה" השורה הראשונה חייבת להיות בדיוק בפורמט: סיכוי: 35%
  נמק לפי הרצון, הדחיפות, הכאב, התקציב, ההתנגדויות ורמת המחויבות.
- ב"ציון כללי מתוך 100" השורה הראשונה חייבת להיות בדיוק בפורמט: ציון: 75/100
  נמק בקצרה אבל בצורה חדה.
- ב"דבר אחד טוב לשמר" כתוב דבר אחד בלבד, ספציפי, עם ציטוט/רגע שמראה אותו.
...
- ב"פרטי השיחה" חובה לכתוב בסוף הדוח:
  - שם איש/אשת המכירות: {salesperson_name}
  - שם הלקוח: אם הוזן שם לקוח, השתמש בו. אם לא הוזן, הסק את שם הלקוח מתוך התמלול. אם אי אפשר להסיק, כתוב "לא זוהה מהשיחה".
- השתמש בטון ישיר, מקצועי, חד ואימוני. לא טון מנומס מדי.
- אם חסר מידע, כתוב מה היה צריך לשאול כדי להשיג אותו.
""".strip()
...
def render_report_dashboard(report: str, report_path: Path) -> None:
    ...
    for section_title in REPORT_SECTION_TITLES:
        if section_title in {"פרטי השיחה", "ניתוח לפי שלבי תסריט המכירה"}:
            continue
